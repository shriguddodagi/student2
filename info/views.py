...
from .models import Dept, Class, Student, Attendance, Course, Teacher, Assign, AttendanceTotal, time_slots, DAYS_OF_WEEK, AssignTime, AttendanceClass, StudentCourse, Marks, MarksClass,leave,feedback,startfeed
...
def create_leave(request):
    if request.method == 'POST':
        if request.POST["title"] and request.POST["start_date"] and request.POST["end_date"] and request.POST["content"]:
            usersn=request.user
            eave = leave()
            eave.user=usersn
            eave.Subject = request.POST["title"]
            eave.From_date = request.POST["start_date"]
            eave.To_date= request.POST["end_date"]
            eave.Reason= request.POST["content"]
            eave.save()
            return redirect('/info/')
        else:
            return render(request,'info/leave.html',{'error':
            {
            'text':'Please fill all required information'
            }})
    else:
        return render(request,'info/leave.html')

@login_required
def app_stats(request):
    context = {"stats": leave.objects.filter(user=request.user)}
    return render(request, 'info/application_status.html', context)    

@login_required
def create_feedback(request):
    if request.method == 'POST':
        if request.POST["tname"] and request.POST["cname"] and request.POST["star"]:
            usersnam=request.user
            fb = feedback()
            fb.student = usersnam
            cs = request.POST["cname"]
            fb.assocourse = Course.objects.get(name=cs)
            tc = request.POST["tname"]
            fb.teacher = Teacher.objects.get(name=tc)
            fb.ratingfield = request.POST["star"]
            fb.suggestions = request.POST["content"]
            fb.postdate = timezone.datetime.now()
            fb.save()
            return redirect('/info/')
        else:
                return render(request,'info/feedback.html',{'error':
                {
                'text':'Please fill all required information'
                }})
    else:
        context={}
        context.update({'teacher':Teacher.objects.all()})
        context.update({'courses':Course.objects.all()})
        context.update({'feedb':startfeed.objects.all()})
...
def GeneratePdf(request,assign_id):
    ass = get_object_or_404(Assign, id=assign_id)
    # ass = get_object_or_404(Assign, id=request.user)
    sc_list = []
    for stud in ass.class_id.student_set.all():
        a = StudentCourse.objects.get(student=stud, course=ass.course)
        sc_list.append(a)
    # return render(request, 'info/t_report.html', {'sc_list': sc_list})
    pdf = render_to_pdf('info/d_report.html', {'sc_list': sc_list})
